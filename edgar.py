...
# A single stand-in phrase undercounts: not every filing says "the Company", so
# the denominator came out smaller than the numerator and shares exceeded 100%.
# Taking the largest bucket across several near-universal phrases gets much
# closer to the true filing count for each industry and quarter.
BASELINE_PROBES = ['"the Company"', '"fiscal year"', '"United States"',
...
def build_baseline(conn, qs, sleep=0.15, verbose=True):
    ...
    for label, start, end in todo:
        ...
        for probe in BASELINE_PROBES:
            try:
                _t, _c, by_sic = search_quarter(probe, start, end)
            except Exception:
                time.sleep(1)
                continue
            for sic, cnt in by_sic:
                if cnt > best.get(sic, 0):
                    best[sic] = cnt
            time.sleep(sleep)
        for sic, cnt in best.items():
...
def shares(conn, phrase):
    ...
    Raw counts say Finance leads everything; shares say which industries are
    unusually preoccupied with a phrase. The baseline is an estimate rather than
    an exact filing count, so a share is capped at 1.0 and anything at the cap
    means "essentially every filing", not a precise figure.
    """
    base = {}
...
def main():
    ...
    if args.build:
        ...
        print("  The denominator is estimated, so treat these as proportions rather")
        print("  than exact percentages.\n")
