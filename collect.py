...
APART_FLOOR = 0.0        # a phrase sharing nothing at all with any other is
                         # reporting thin coverage, not distance. Every phrase
                         # that scored zero has turned out to be wrong.
...
APART_CUT = 0.10         # a fixed line as well as a rank: the median drifts a
                         # couple of points between runs, and a phrase should
                         # not gain or lose the mark without having changed
APART_MOST = 5           # how many phrases may carry the mark, lowest first
APART_SHARE = 0.5        # and none of them may be near the typical phrase
...
def mark_apart(conn, out, recent_start, shown_terms):
    ...
    eligible = [(term, s) for term, s in ranked if s > APART_FLOOR]
    keep = {term for term, s in eligible[:APART_MOST] if s <= ceiling}
    marked = 0
    for t in out:
        s = scores.get(t["term"])
        if s is None:
