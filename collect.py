...
APART_CUT = 0.04         # a fixed line as well as a rank: the median drifts a
                         # couple of points between runs, and a phrase should
                         # not gain or lose the mark without having changed
APART_MOST = 2           # how many phrases may carry the mark, lowest first
APART_SHARE = 0.25        # and none of them may be near the typical phrase
...
def mark_apart(conn, out, recent_start, shown_terms):
    ...
    # the lowest few, not everything under a moving line: a phrase should not
    # gain or lose the mark because the corpus shifted around it
    ranked = sorted(scores.items(), key=lambda kv: kv[1])
    ceiling = min(mid * APART_SHARE, APART_CUT)
    # a phrase sharing nothing at all with any other is reporting thin
    # coverage rather than distance, so zero does not qualify
    keep = {term for term, s in ranked[:APART_MOST] if s <= ceiling}
    marked = 0
    for t in out:
        s = scores.get(t["term"])
        if s is None:
        ...
        if t["term"] in keep and t.get("display", True):
            t["apart"] = True
            marked += 1
    rows = sorted((t for t in out if t.get("nearShare") is not None
                   and t.get("display", True)), key=lambda t: t["nearShare"])
    print(f"  shared surroundings · typical {mid:.0%}, "
          f"lowest {APART_MOST} marked if under {ceiling:.0%} · "
          + ", ".join(f"{t['term']} {t['nearShare']:.0%}"
                      + ("*" if t.get("apart") else "") for t in rows[:10]))
    return marked


def pair_rarity(conn, window_start):
    """For each pair of sectors, the share of shared grams that is theirs.

    A gram appearing in two sectors is one instance of those desks reaching
    for the same words. Counting every such instance says which pairings are
    routine and which are unusual, without needing either sector to have a
    large vocabulary of its own.
    """
    where = defaultdict(set)
    for gram, sector in conn.execute(
            "SELECT gram, sector FROM grams WHERE day>=? "
            "GROUP BY gram, sector HAVING SUM(articles) >= 2", (window_start,)):
        where[gram].add(sector)
    pairs = Counter()
    for secs in where.values():
        if 2 <= len(secs) <= 6:          # in everything is vocabulary, not a crossing
            for a, b in combinations(sorted(secs), 2):
                pairs[(a, b)] += 1
    total = sum(pairs.values()) or 1
    return {k: v / total for k, v in pairs.items()}, pairs
...
def build(conn):
    ...
    share, counts = pair_rarity(conn, window_start)
    if counts:
        ranked = sorted(counts.items(), key=lambda kv: kv[1])
        rare = ", ".join(f"{a}/{b} {n}" for (a, b), n in ranked[:4])
        common = ", ".join(f"{a}/{b} {n}" for (a, b), n in ranked[-3:])
        print(f"  how often two desks share wording · {len(counts)} pairs seen · "
              f"routine: {common} · unusual: {rare}")
    ...
    if share:
        lines = []
        for t in shown:
            names = sorted({s["name"] for s in t["sectors"]})
            got = [share.get((a, b)) for a, b in combinations(names, 2)]
            got = [g for g in got if g is not None]
            if got:
                lines.append((min(got), t["term"]))
        lines.sort()
        print("  least routine pairing each phrase spans · " + ", ".join(
            f"{term} {v*100:.2f}%" for v, term in lines[:8]))

    shown_terms = [t["term"] for t in shown]
