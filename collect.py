...
ONE_FEED_HARD = 0.65     # this much from one newsroom is that newsroom's own
...
SURPRISE_FULL = 0.34     # a pair sharing two phrases or fewer counts as a
                         # full surprise; beyond that the weight tapers
...
COOC_FEED_SHARE = 0.6    # a pairing this concentrated in one newsroom, with
...
COOC_KEEP_PER_GRAM = 5   # partners retained per phrase per day; the rest are
...
def pair_surprise(out):
    """How unusual it is for each pair of sectors to share a phrase at all.

    Built from the corpus itself rather than assumed, so it follows whatever
    the feed mix happens to be. Neighbours that share a vocabulary score low;
    sectors that rarely meet score high.
    """
    seen = Counter()
    for t in out:
    ...
    for t in out:
        ...
        if len(names) < 2:
            t["surprise"] = 1.0
            continue
        # the least surprising pair it spans sets the tone: a phrase is only
        # as remarkable as its most ordinary crossing
        t["surprise"] = round(min(1 / (1 + seen[(a, b)])
                                  for a, b in combinations(names, 2)), 3)
    return seen
...
def build(conn):
    ...
    for t in out:
        ...
        odd = 0.6 + 0.4 * min(1.0, (t.get("surprise") or 0.05) / SURPRISE_FULL)
    ...
    shown = (spread + held)[:SHOW_TERMS]
    for t in (spread + held)[SHOW_TERMS:]:
        t["display"] = False
        pool.append(t)
    out = shown + pool
    ...
    odd_ones = sorted((t for t in out if t.get("display", True) and t.get("surprise")),
                      key=lambda t: -t["surprise"])[:5]
    if odd_ones:
        print("  least ordinary crossings · " + ", ".join(
            f"{t['term']} ({'/'.join(s['name'] for s in t['sectors'][:2])})"
            for t in odd_ones))
    if dense:
...
def main():
    ...
    print(f"  database {size_mb:.1f} MB"
          + ("   WARNING: approaching GitHub's 100 MB file limit" if size_mb > 90 else ""))
