...
PECULIAR_OVERLAP = 1.0   # a sector pair sharing less vocabulary than a typical
                         # pair counts as distant. Phrases cross between related
                         # sectors by nature, so most sit above this.
PECULIAR_SPECIFIC = 0.6  # a phrase must be this far from common vocabulary
                         # before its sector spread is worth remarking on
...
def mark_peculiar(out, overlap, verbose=False):
    """Specific phrases crossing between trade presses that share little.

    Two things a reader can check: the phrase is not common vocabulary, and the
    sectors carrying it do not usually talk about the same things. Neither is a
    claim about importance — only that this pairing is unusual for these desks.
    """
    if not overlap:
        return 0
    ...
    if mid <= 0.002:
        if verbose:
            print("  peculiar not assessed: sectors share too little vocabulary "
                  "anywhere to compare against")
        return 0
    # less than a typical pair of sectors shares
    cut = mid * PECULIAR_OVERLAP
    n = 0
    unmeasured = []
    for t in out:
        ...
        if len(names) < 2 or t.get("specificity", 0) < PECULIAR_SPECIFIC:
            continue
        pairs = [overlap[(a, b)] for a, b in combinations(names, 2)
                 if (a, b) in overlap]
        if len(pairs) != len(names) * (len(names) - 1) // 2:
            unmeasured.append(t["term"])   # a pair we have no reading for
            continue
        t["overlapAt"] = round(max(pairs), 4)
        if max(pairs) <= cut:
            t["peculiar"] = True
            n += 1
    if verbose:
        near = sorted((t for t in out if t.get("overlapAt") is not None
                       and not t.get("peculiar") and t.get("display", True)),
                      key=lambda t: t["overlapAt"])[:4]
        spec_fail = sum(1 for t in out if t.get("display", True)
                        and len({s["name"] for s in t["sectors"]}) > 1
                        and t.get("specificity", 0) < PECULIAR_SPECIFIC)
        print(f"  peculiar needs specificity >= {PECULIAR_SPECIFIC} "
              f"and overlap <= {cut:.1%}"
              + (f" · {spec_fail} shown phrases fail on specificity" if spec_fail else ""))
        rows = sorted((t for t in out if t.get("overlapAt") is not None
                       and t.get("display", True)),
                      key=lambda t: t["overlapAt"])
        if rows:
            print("  sector distance, every shown phrase · " + ", ".join(
                f"{t['term']} {t['overlapAt']:.1%}"
                + ("*" if t.get("peculiar") else "") for t in rows))
    return n
...
def build(conn):
    ...
    n_odd = mark_peculiar(out, overlap, verbose=True)
    if n_odd:
        print(f"  {n_odd} marked peculiar "
              f"(specific, and spanning trade presses that share little)")
    ...
    for t in out:
        ...
        # the quota exists to make room for the unusual; a phrase that is
        # unusual should not then be turned away by it
        if t.get("peculiar") or lead_count[key] < (MAX_PER_BLOC if key in SECTOR_BLOCS
                                                   else MAX_PER_SECTOR):
            lead_count[key] += 1
            spread.append(t)
        else:
    ...
    refill = [t for t in held
              if next((b for b in SECTOR_BLOCS
                       if max(t["sectors"], key=lambda s: s["articles"])["name"] in b),
                      None) is None]
