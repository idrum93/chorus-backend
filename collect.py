    ...
    if moving:
        for t in moving[:10]:
            ...
            L.append(f"**[{t['term']}]({SITE}/#{t['id']})** \u2014 "
                     ...
                     f"{t['_growth']:.1f}\u00d7 the fortnight before, "
                     f"in {secs}")
            L.append("")
    else:
...
def build(conn):
    ...
    # what each gate costs, so a decision to move one is not a guess
    _one_sector = {g for (g,) in conn.execute(
        "SELECT gram FROM grams WHERE day>=? GROUP BY gram "
        "HAVING SUM(articles) >= ? AND COUNT(DISTINCT sector) = 1",
        (window_start, COOC_MIN_ARTICLES))}
    _fewer_arts = {g for (g,) in conn.execute(
        "SELECT gram FROM grams WHERE day>=? GROUP BY gram "
        "HAVING SUM(articles) IN (?, ?) AND COUNT(DISTINCT sector) >= ?",
        (window_start, COOC_MIN_ARTICLES - 1, COOC_MIN_ARTICLES - 2,
         COOC_MIN_SECTORS))}
    print(f"  companion pool · {len(recurring):,} eligible · "
          f"{len(_one_sector):,} more if one sector were enough · "
          f"{len(_fewer_arts):,} more at {COOC_MIN_ARTICLES - 2} articles")
