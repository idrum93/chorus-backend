...
COOC_KEEP_PER_GRAM = 10  # partners retained per phrase per day; the rest are
...
def build(conn):
    ...
    # Sum across every trigram that contains the bigram: "battery electric"
    # splits between "battery electric van" and "battery electric truck", and
    # neither alone accounts for it while together they account for all of it.
    ...
    for gram in candidates:
        if gram in frags or gram in swallowed:
            dropped_frags += 1
            continue
        ...
        for o, n, lf in lifted:
            ...
            if any(len(ow & set(k.split())) / min(len(ow), len(k.split())) >= 0.5
                   for k, _, _ in deduped):
                continue          # "cargo vessel" beside "commercial vessel"
...
def main():
    ...
    try:
        ...
        for ticker, n in rows:
            ...
            for a in attached:
                aw = set(a["term"].split())
                if any(len(aw & set(k["term"].split())) / min(len(aw), len(k["term"].split())) >= 0.6
                       for k in merged):
                    continue
                merged.append(a)
