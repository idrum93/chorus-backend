...
SECTOR_MIN_EFFECTIVE = 1.9  # and must behave like nearly two sectors in absolute
...
COOC_MIN_LIFT = 1.8      # a companion must be distinctive to this phrase, not merely common
COOC_MIN_ARTICLES = 4    # a companion must recur; once is a sentence, not a topic
COOC_MIN_NEW = 4         # a first appearance carries no comparison, so it needs
                         # more articles of its own before it is worth showing
...
def main():
    ...
    try:
        rows = list(conn.execute(
            ...
            "FROM orgs WHERE day>=? GROUP BY name HAVING SUM(n)>=3 "
