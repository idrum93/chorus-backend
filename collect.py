...
CO_MIN_PHRASE_SECTORS = 2 # at least one attached phrase must reach this many
                          # sectors: the tab is companies named in language that
                          # is crossing, not companies named a lot
...
def main():
    ...
    try:
        ...
        for ticker, n in rows:
            ...
            if not any(a["sectors"] >= CO_MIN_PHRASE_SECTORS for a in attached):
                continue          # named only in its own sector's vocabulary
            companies.append({
                "prior14d": prior_n,
