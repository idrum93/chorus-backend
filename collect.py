...
def mark_peculiar(out, overlap):
    ...
    if mid <= 0.01:
        return 0        # too little shared vocabulary anywhere to compare against
    ...
    for t in out:
        ...
        # every pair it spans must be a distant one, not just the best of them
        if pairs and max(pairs) <= cut:
            t["peculiar"] = True
...
def build(conn):
    ...
    n_odd = mark_peculiar(out, overlap)
    if n_odd:
