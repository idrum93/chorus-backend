...
APART_CUT = 0.05         # a fixed line as well as a rank: the median drifts a
...
def coherence(t):
    """How much vocabulary the articles carrying this phrase share.

    Three articles about one subject repeat each other's words. Three that
    merely contain the same phrase do not. Averaged over every pair, ignoring
    the phrase's own words so it cannot vouch for itself.
    """
    own = set(t["term"].split())
    docs = []
    for a in (t.get("examples") or []):
        words = {w for w in re.findall(r"[a-z]{4,}",
                                       f"{a.get('title','')} {a.get('quote','')}".lower())
                 if w not in STOP and w not in own}
        if len(words) >= 4:
            docs.append(words)
    if len(docs) < 3:
        return None
    ...
    for i in range(len(docs)):
        for j in range(i + 1, len(docs)):
            ...
            scores.append(len(docs[i] & docs[j]) / union if union else 0.0)
...
def build(conn):
    ...
    for t in out:
        t["coherence"] = coherence(t)
    coh = sorted((t for t in out if t.get("display", True)
                  and t.get("coherence") is not None),
                 key=lambda t: t["coherence"])
    if coh:
        print("  do a phrase's own articles share wording · " + ", ".join(
            f"{t['term']} {t['coherence']:.0%}" for t in coh[:6])
            + f" · typical {coh[len(coh) // 2]['coherence']:.0%}")
