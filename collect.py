...
def build(conn):
    ...
    for t in out:
        if not t.get("display", True):
            continue
        # the emitted cooc list is a curated half-dozen, so a top-three share
        # is 100% for nearly everything; the table underneath has the company
        cs = [n for (n,) in conn.execute(
            "SELECT SUM(n) FROM cooc WHERE gram=? AND day>=? "
            "GROUP BY other ORDER BY 1 DESC LIMIT 40",
            (t["term"], recent_start))]
        if len(cs) >= 8:
            grip_rows.append((sum(cs[:3]) / (sum(cs) or 1), t["term"], len(cs)))
    if grip_rows:
        grip_rows.sort()
        loose = ", ".join(f"{n} {v:.0%}/{k}" for v, n, k in grip_rows[:5])
        tight = ", ".join(f"{n} {v:.0%}/{k}" for v, n, k in grip_rows[-3:])
        print(f"  company kept · share of pairings held by a phrase's top three "
              f"partners · loosest: {loose} · tightest: {tight}")
