#!/usr/bin/env python3
"""
collect.py — Chorus collector, v2.
...
UA     = "chorus-monitor/2.0 (news language monitoring)"
KEY    = os.environ.get("NEWSAPI_AI_KEY", "").strip()

WINDOW_DAYS, RECENT_DAYS = 90, 14
MIN_ARTICLES, MIN_SECTORS, MAX_TERMS = 5, 2, 80
...
with would you your yours yourself says said say new news first last week year years day days time
...
def store(conn, articles, sector, source, backfilled=False):
    ...
    for a in articles:
        ...
        conn.execute("INSERT INTO articles VALUES(?,?,?,?,?,?,?,?,?,?)", (
            ...
            a["publisher"] or host(a["url"]), sector, source, day, now, int(backfilled)))
        added += 1
        by_day_totals[day] += 1
...
def build(conn):
    ...
    candidates = [r[0] for r in conn.execute(
        ...
        "HAVING a>=? ORDER BY a DESC LIMIT 4000", (recent_start, MIN_ARTICLES))]
    ...
    out.sort(key=lambda t: (-t["sectorCount"], t["daysSinceNewestSector"]))
