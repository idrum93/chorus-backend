...
SHOW_TERMS = 20          # how many reach the phrase list; the rest still feed
...
STOCK_PHRASES = {
    ...
    "data management", "risk management", "project management",
    ...
    "infrastructure investment", "capital investment", "investment plan",
    "annual capacity", "annual production", "annual revenue", "annual output",
    "production capacity", "installed capacity", "nameplate capacity",
    "purchase agreement", "sale agreement", "service agreement",
    "definitive agreement", "framework agreement",
    "speaking on condition", "on condition of", "according to people",
    "operating environment", "industrial application", "commercial application",
    "operating model", "business environment", "regulatory environment",
...
PREPOSITIONS = set("""
of in on at by for with from into onto over under about across against amid
among around before behind below beneath beside between beyond during except
inside near outside since through throughout toward towards until upon within
without via per than as
""".split())


def subject_share(conn, gram, since, cap=60):
    """How often the phrase acts rather than being acted upon.

    A subject governs a verb; a frame follows a preposition or is followed by
    "of" — an annual capacity OF, IN the operating environment, AMID
    geopolitical tensions. Positional, so it needs no tagger.
    """
    pat = gap_pattern(gram)
    subj = seen = 0
    for title, summary in conn.execute(
            "SELECT title, summary FROM articles WHERE published>=? "
            "AND LOWER(title || '. ' || COALESCE(summary,'')) LIKE ? LIMIT ?",
            (since, like_forms(gram)[0], cap)):
        text = f"{title}. {summary or ''}"
        for m in pat.finditer(text):
            words = re.findall(r"[a-z']+", text[:m.start()].lower())
            after = re.findall(r"[a-z']+", text[m.end():].lower())
            before = words[-1] if words else ""
            # a determiner sits between a preposition and the phrase:
            # "in THE operating environment"
            if before in ("the", "a", "an", "its", "their", "our") and len(words) > 1:
                before = words[-2]
            seen += 1
            if before not in PREPOSITIONS and (not after or after[0] != "of"):
                subj += 1
    return (subj / seen if seen else None), seen
...
def build(conn):
    ...
    KNOWN_BAD = ["annual capacity", "operating environment", "local time",
                 "customer experience", "digital asset", "geopolitical tension",
                 "energy management", "purchase agreement", "industrial application",
                 "infrastructure investment", "people familiar"]
    check = []
    for g in KNOWN_BAD:
        v, n = subject_share(conn, g, recent_start)
        if v is not None and n >= 3:
            check.append((v, g, n))
    if check:
        check.sort()
        print("  against phrases already removed by hand · " + ", ".join(
            f"{g} {v:.0%}/{n}" for v, g, n in check))

    subj_rows = []
    for t in out:
        if not t.get("display", True):
            continue
        v, n = subject_share(conn, t["term"], recent_start)
        if v is not None and n >= 5:
            subj_rows.append((v, t["term"], n))
    if subj_rows:
        subj_rows.sort()
        low = ", ".join(f"{n} {v:.0%}" for v, n, _ in subj_rows[:5])
        high = ", ".join(f"{n} {v:.0%}" for v, n, _ in subj_rows[-4:])
        print(f"  acts or is acted upon · share of uses where the phrase is not "
              f"a complement · lowest: {low} · highest: {high}")
