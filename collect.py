...
def strip_masthead(text, publisher):
    ...
    """
    if not publisher or not text:
        return text
    out = re.sub(re.escape(publisher), " ", text, flags=re.I)
    out = re.sub(r"\s+[-|–—]\s+" + re.escape(publisher) + r"\s*$", " ", out, flags=re.I)
...
def main():
    ...
    bad = list(conn.execute(
        ...
        "GROUP BY sector, note HAVING good=0 AND note!='' AND n>=3 ORDER BY n DESC LIMIT 12"))
    if bad:
