...
def sector_overlap(conn, window_start):
    ...
    for sector, gram in conn.execute(
            "SELECT sector, gram FROM grams WHERE day>=? "
            "GROUP BY sector, gram HAVING SUM(articles) >= 2", (window_start,)):
        vocab[sector].add(gram)
    out = {}
...
def mark_peculiar(out, overlap, verbose=False):
    ...
    if mid <= 0.01:
        ...
        return 0
    # a pair sharing less than half what a typical pair shares
