...
PECULIAR_OVERLAP = 0.5   # a sector pair sharing less than half the vocabulary
                         # a typical pair does counts as distant
...
def mark_peculiar(out, overlap, verbose=False):
    ...
    if mid <= 0.002:
        ...
        return 0
    # the quarter of pairs that share least, and never above the median
    cut = min(vals[max(0, n // 4 - 1)], mid * 0.9)
    n = 0
    for t in out:
