...
def build(conn):
    ...
    try:
        nb = [n for (n,) in conn.execute(
            "SELECT COUNT(DISTINCT other) c FROM cooc WHERE day>=? AND gram IN "
            "(" + ",".join("?" * len(shown_terms)) + ") GROUP BY gram ORDER BY c",
            [recent_start] + shown_terms)] if shown_terms else []
        if nb:
            mid_nb = nb[len(nb) // 2]
            shared = conn.execute(
                "SELECT COUNT(*) FROM (SELECT other FROM cooc WHERE day>=? "
                "AND gram IN (" + ",".join("?" * len(shown_terms)) + ") "
                "GROUP BY other HAVING COUNT(DISTINCT gram) > 1)",
                [recent_start] + shown_terms).fetchone()[0]
            print(f"  co-occurrence neighbourhood · {min(nb)}\u2013{max(nb)} words "
                  f"per phrase, typical {mid_nb}; {shared} words are shared by "
                  f"more than one shown phrase")
    except sqlite3.OperationalError:
        pass
