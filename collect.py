...
def build(conn):
    ...
    for t in out:
        ...
        if t["docFreq"] >= COMMON_CEILING:
            ...
            everywhere += 1
        elif (t.get("sinceStart") and t["sectors"]
              and all(s.get("atSectorStart") for s in t["sectors"])):
            t["display"] = False        # in every sector before we were looking
            already += 1
    ...
    for t in out:
        ...
        fam = family_of(t)
        # a phrase the list has singled out should not then be displaced by a
        # quota meant to make room for exactly that kind of phrase
        if t.get("apart") or (lead_count[key] < (MAX_PER_BLOC if key in SECTOR_BLOCS
                                                 else MAX_PER_SECTOR)
    ...
    if already:
        print(f"  {already} set aside as present before collection began "
              f"(no crossing was observed, only that both sectors use it)")
    else:
        near = [t for t in out if t.get("display", True) and t.get("sinceStart")]
        if near:
            print("  none present from the start · " + ", ".join(
                f"{t['term']} "
                f"{sum(1 for s in t['sectors'] if s.get('atSectorStart'))}"
                f"/{len(t['sectors'])} sectors at start" for t in near[:5]))
        else:
            fs = sorted((t["sectors"][0].get("firstSeen"), t["term"])
                        for t in out if t.get("display", True) and t["sectors"])
            cut = max(horizon, add_days(window_start, 2))
            print(f"  none present from the start · cut {cut}, window opens "
                  f"{window_start}, retention reaches {horizon}")
            if fs:
                print("  earliest firstSeen among shown · " + ", ".join(
                    f"{n} {d}" for d, n in fs[:5] if d))
    if places:
