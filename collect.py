...
CO_MIN_RECENT = 2         # a company with no phrases and one article is a name
                          # match rather than coverage worth listing
ORG_MIN_ARTICLES = 3      # an agency needs this many mentions to appear at all,
ORG_MIN_SINGLE_SECTOR = 6 # and this many if only one sector is naming it
MAX_PER_SECTOR = 4        # how many phrases may lead with the same sector before
...
SECTOR_MIN_ARTICLES = 2   # a sector must carry a phrase this often to count toward
...
def pick_companions(uniq, known):
    """Recurring phrases first, alphabetical only to fill the remaining slots.

    Selecting alphabetically chose a companion for starting with "a" rather than
    for meaning anything, which is why the panel filled with sentence fragments.
    """
    cands = [g for g in uniq if " " in g]
    seen = sorted(g for g in cands if g in known)
    rest = sorted(g for g in cands if g not in known)
    return (seen + rest)[:COOC_PHRASES]
...
def build(conn):
    ...
    corpus_articles = conn.execute(
        "SELECT SUM(articles) FROM totals WHERE day>=?", (window_start,)).fetchone()[0] or 1

    recurring = {g for (g,) in conn.execute(
        "SELECT gram FROM grams WHERE day>=? GROUP BY gram "
        "HAVING SUM(articles) >= ?", (window_start, COOC_MIN_ARTICLES))}
    ...
    for gram in candidates:
        ...
        if eff / len(sectors) < SECTOR_EVENNESS: continue
        if eff < SECTOR_MIN_EFFECTIVE: continue
    ...
    for t in out:
        ...
        for k in kept:
            if t["term"] in k["term"] and len(t["term"]) < len(k["term"]):
                if t["articles14d"] <= k["articles14d"] * 3.0:
                    redundant = True
                    break
        if redundant:
            drop.add(t["term"])
...
def main():
    ...
    try:
        ...
        for ticker, n in rows:
            ...
            if recent_n < CO_MIN_RECENT and not attached:
                continue
            companies.append({
                "prior14d": prior_n,
    ...
    try:
        rows = list(conn.execute(
            ...
            "HAVING SUM(n) >= ? AND (COUNT(DISTINCT sector) > 1 OR SUM(n) >= ?) "
            "ORDER BY s DESC, n DESC LIMIT 30",
            (win_start, ORG_MIN_ARTICLES, ORG_MIN_SINGLE_SECTOR)))
