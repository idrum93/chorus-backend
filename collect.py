...
def store(conn, articles, sector, source, backfilled=False):
    ...
    for a in articles:
        ...
        if skey and conn.execute(
                "SELECT 1 FROM articles WHERE story=? AND published>=?",
                (skey, (datetime.fromisoformat(day).date()
                        - timedelta(days=3)).isoformat())).fetchone():
            syndicated += 1
            continue
...
def drop_syndicated(conn, verbose=True):
    ...
    rows = list(conn.execute(
        "SELECT story, MIN(published) FROM articles "
        "WHERE story IS NOT NULL AND story <> '' "
        "GROUP BY story HAVING COUNT(*) > 1"))
    removed = 0
    for skey, first in rows:
        keep = conn.execute(
            "SELECT id FROM articles WHERE story=? ORDER BY published, id LIMIT 1",
            (skey,)).fetchone()[0]
        cur = conn.execute(
            "DELETE FROM articles WHERE story=? AND id<>? AND published<=?",
            (skey, keep, (datetime.fromisoformat(first).date()
                          + timedelta(days=3)).isoformat()))
        removed += cur.rowcount
    if removed:
        conn.commit()
        if verbose:
            print(f"  {removed} duplicate copies removed from earlier collection")
    return removed
...
def main():
    ...
    drop_syndicated(conn)          # before anything is counted, not after
