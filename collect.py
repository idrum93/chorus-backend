...
COOC_DAYS    = 24        # co-occurrence is only ever shown for recent windows
COOC_KEEP_PER_GRAM = 20  # partners retained per phrase per day; the rest are
...
def prune(conn, verbose=True):
    ...
    # A pair seen once on one day is the shape of one sentence, and pairs are
    # the largest table by far. Companions require four articles anyway, so
    # single sightings older than the recent window can never contribute.
    conn.execute("DELETE FROM cooc WHERE n = 1 AND day < ?",
...
def build(conn):
    ...
    if dropped_frags:
        print(f"  {dropped_frags} phrases were fragments of institution names")
