...
MIN_HOME_SHARE = 0.28    # a phrase with less than this in its busiest sector
                         # never had a home, so it cannot be crossing out of one
HOME_WEIGHT = 1.5        # how hard to favour a phrase that still has a home
                         # sector over one used evenly everywhere
SHOW_TERMS = 40          # how many reach the phrase list; the rest still feed
...
sharp steep sudden modest slight duty national sovereign long short heavy
...
def build(conn):
    ...
    for t in out:
        ...
        if home_share < MIN_HOME_SHARE:
            t["display"] = False        # no home to leave; vocabulary, not crossing
            homeless += 1
    ...
    if homeless:
        print(f"  {homeless} set aside as general vocabulary "
              f"(under {MIN_HOME_SHARE:.0%} of their coverage in any one sector)")
...
def main():
    ...
    try:
        ...
        for nm, ac, nsec, n, first in rows:
            ...
            arts = [{"title": mark_in(ti, nm), "url": u, "publisher": p, "sector": s,
