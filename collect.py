...
growing building sign signs signed next firm firms unit units
...
anyone everyone someone anything everything nothing nobody somebody
...
def institutions(summary):
    ...
    for m in ENTITY.finditer(summary):
        ...
        if not any(w in INSTITUTION_WORDS for w in words):
            continue
        # a capitalised run that begins a sentence proves nothing on its own
        if SENT_START.search(summary[:m.start()]) and len(words) < 3:
            continue
        ...
        if key in seen:
            continue
        seen.add(key)
        out.append((name, ac.group(1) if ac else None))
...
def company_vocabulary(companies):
    ...
    distinctive word is enough to mark a phrase as a company name, which is what
    ...
    for c in companies:
        ...
        for w in WORD.findall(forms.lower()):
            if len(w) > 3 and w not in ordinary:
                words.add(w)
...
def rebuild(conn, verbose=True):
    ...
    for (day, g, cc), n in cc_n.items():
        conn.execute("INSERT OR REPLACE INTO gramcc VALUES(?,?,?,?)", (day, g, cc, n))
    for (day, sector, nm), (n, ac) in org_n.items():
        conn.execute("INSERT OR REPLACE INTO orgs VALUES(?,?,?,?,?)", (day, sector, nm, ac, n))
...
def write_digest(terms, path, stored, days_held):
    ...
    L = [f"# Crosstalk — week ending {today:%d %B %Y}", "",
         f"*{stored:,} articles from {len(set(s['name'] for t in terms for s in t['sectors']))} "
         f"sectors over {days_held} days. Every figure is a count of articles read; "
         f"nothing is modelled or forecast.*", ""]

    L += ["## Crossed into a new industry this week", ""]
    if crossed:
        for t in crossed[:8]:
            when = "today" if t["daysSinceNewestSector"] == 0 else f"{t['daysSinceNewestSector']}d ago"
            first = min(t["sectors"], key=lambda s: s["firstSeen"])
            L.append(f"**{t['term']}** — now in {t['sectorCount']} industries. "
                     f"**{t['newestSector']}** picked it up {when}; it started in "
                     f"{first['name'].lower()} coverage on {first['firstSeen']}.")
            for e in t["examples"][:2]:
                L.append(f"  - [{e['title']}]({e['url']}) · {e['publisher']} · {e['sector'].lower()}")
            L.append("")
    else:
        L += ["Nothing crossed into a new industry this week.", ""]

    L += ["## Being used more than a fortnight ago", ""]
    if moving:
        L += ["| phrase | industries | articles | vs prior fortnight |",
              "|---|---|---|---|"]
        for t in moving[:10]:
    ...
    L += ["---", "",
          "Crosstalk tracks which phrases appear in more than one industry's trade press "
          "and when each industry started using them. It makes no forecast — testing found "
...
def main():
    ...
    try:
        coverage = [[cc, n] for cc, n in conn.execute(
            ...
            "WHERE a.published>=? GROUP BY c ORDER BY 2 DESC", (window_start,))]
    except sqlite3.OperationalError:
        pass
    ...
    try:
        rows = list(conn.execute(
            ...
            "FROM orgs WHERE day>=? GROUP BY name HAVING n>=2 "
            "ORDER BY s DESC, n DESC LIMIT 60", (window_start,)))
        for nm, ac, nsec, n, first in rows:
            secs = [r[0] for r in conn.execute(
                "SELECT sector, SUM(n) FROM orgs WHERE name=? AND day>=? "
                "GROUP BY sector ORDER BY 2 DESC", (nm, window_start))]
            recent = conn.execute(
                "SELECT SUM(n) FROM orgs WHERE name=? AND day>=?",
                (nm, days[-RECENT_DAYS])).fetchone()[0] or 0
            prior = conn.execute(
                "SELECT SUM(n) FROM orgs WHERE name=? AND day<? AND day>=?",
                (nm, days[-RECENT_DAYS], window_start)).fetchone()[0] or 0
            orgs_list.append({
    ...
    crossed, moving = write_digest(terms, os.path.join(HERE, "DIGEST.md"),
                                   stored, days_held)
    print(f"  Wrote DIGEST.md — {crossed} crossed a new industry, {moving} moving")
    return 0
