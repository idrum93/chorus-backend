...
apac emea latam central australia hong kong korea taiwan japan india china
...
def rebuild(conn, verbose=True):
    ...
    # first pass: how often each phrase occurs across the whole corpus
    freq = Counter()
    for title, summary, sector, day, url, pub, cc in rows:
        freq.update(set(ngrams(strip_masthead(clean(title), pub) + ". "
