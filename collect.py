...
GRIP_FULL = 0.02         # a phrase carried by 2% of a sector's own fortnight
...
def build(conn):
    ...
    sector_volume = {s: n for s, n in conn.execute(
        "SELECT sector, COUNT(*) FROM articles WHERE published>=? GROUP BY sector",
        (recent_start,))}
    ...
    for gram in candidates:
        ...
        for s in sectors:
            ...
            # what fraction of everything that sector published this fortnight
            # carried the phrase: three articles is a trace in Energy and a
            # noticeable share in Waste & water
            base = sector_volume.get(s["name"], 0)
            s["grip"] = round(s["recent"] / base, 5) if base else 0.0
    ...
    if gripped:
        print("  biggest share of a sector's own output · " + ", ".join(
            f"{t['term']} {max((s.get('grip') or 0) for s in t['sectors']):.1%}"
            for t in gripped))
