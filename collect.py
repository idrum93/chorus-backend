...
ONE_FEED_SHARE = 0.6     # a phrase this concentrated in one publisher, with few
...
def like_forms(phrase):
    """The phrase as SQL LIKE patterns, spaced and hyphenated.

    Articles write "solid-state battery"; the gram is "solid state battery".
    Trying both means the precise query succeeds instead of falling through to
    a loose one that has to be filtered afterwards.
    """
    ws = phrase.split()
    out = {" ".join(ws)}
    if len(ws) > 1:
        ...
        for i in range(len(ws) - 1):
            out.add(" ".join(ws[:i] + ["-".join(ws[i:i + 2])] + ws[i + 2:]))
    return [f"%{f}%" for f in sorted(out)]


def gap_pattern(phrase):
    """The phrase as written, allowing hyphens or slashes between its words.

    "solid state" has to find "solid-state"; the gram was folded, the article
    was not.
    """
    return re.compile(
...
def build(conn):
    ...
    for gram in candidates:
        ...
        direction = ("new" if prior == 0 else "rising" if r_share > p_share * 1.35
                     else "falling" if r_share < p_share * 0.7 else "steady")
        ...
        def articles_for(patterns, limit=12):
            clause = " AND ".join(
                ["LOWER(title || '. ' || COALESCE(summary,'')) LIKE ?"] * len(patterns))
            return list(conn.execute(
                "SELECT publisher, title, url, sector, COALESCE(country,'US'), summary "
                "FROM articles "
                f"WHERE published>=? AND {clause} ORDER BY published DESC LIMIT ?",
                tuple([recent_start] + patterns + [limit])))
    ...
    homeless = bunched = everywhere = oneshop = oneevent = 0
    ...
    for t in out:
        ...
        if t["docFreq"] >= COMMON_CEILING:
            ...
            oneevent += 1
              ...
              and t["topPubShare"] > ONE_FEED_SHARE
              and 0 < t["publishers"] < 4):
        ...
        t["rank"] = round(t["effectiveSectors"] * (0.35 + 0.65 * t["specificity"])
                          * (home_share ** HOME_WEIGHT) * fresh, 3)
