...
def main():
    ...
    stale = stale_counts(conn)
    if stale or args.rebuild:
        if stale:
            print(f"  {stale} single-word rows found — left over from an older"
                  " extraction. Recomputing from stored articles.")
        rebuild(conn)
