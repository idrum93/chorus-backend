...
def snippet(title, summary, phrase, width=110):
    ...
    if i < 0:
        return ""
    if title and p in title.lower():
        return ""                       # already visible in the headline
    start = max(0, i - width // 3)
    end = min(len(text), i + len(p) + width)
    out = text[start:end]
    # mark the phrase where it sits, keeping the publisher's own capitalisation
    rel = i - start
    out = out[:rel] + "\u2f00" + out[rel:rel + len(p)] + "\u2f01" + out[rel + len(p):]
    out = out.strip()
