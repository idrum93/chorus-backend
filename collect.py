...
COOC_PHRASES = 14        # pairs grow with the square: 12 gives 66 per article,
...
COOC_DAYS    = 30        # a fortnight against the fortnight before it needs 28
                         # days present. Retaining 21 left the prior window half
                         # empty, which made phrases read as first appearances
COOC_KEEP_PER_GRAM = 24  # partners retained per phrase per week; the rest are
...
def week_of(day):
    ...
    Co-occurrence is only ever read as two fortnights compared with each other,
    so storing a row per day writes seven times what any query needs. Bucketing
    by week keeps the comparison exact — a fortnight is two whole weeks — and
    cuts the largest table by about five sixths.
    """
    d = datetime.fromisoformat(day[:10]).date()
    return (d - timedelta(days=d.weekday())).isoformat()


def fortnights(today=None):
    """(recent, prior, span) — two complete weeks against the two before them.

    The current week is deliberately excluded. A week in progress holds a day
    or two of coverage, so including it would make every phrase look like a
    decline on a Monday and a surge on a Sunday. `span` gives the first and
    last day the recent buckets cover, so counts compared against them measure
    the same stretch of time.
    """
    d = datetime.fromisoformat(today).date() if today else datetime.now(timezone.utc).date()
    this_monday = d - timedelta(days=d.weekday())
    weeks = [(this_monday - timedelta(weeks=i)).isoformat() for i in range(1, 5)]
    recent, prior = weeks[:2], weeks[2:]
    start = recent[-1]
    end = (datetime.fromisoformat(recent[0]).date() + timedelta(days=6)).isoformat()
    return recent, prior, (start, end)
    ...
    for a in articles:
        ...
        for i, x in enumerate(phrases):
            for y in phrases[i+1:]:
                by_day_cooc[week_of(day)][(x, y) if x < y else (y, x)] += 1
...
def build(conn):
    ...
    _, _, (cw_start, cw_end) = fortnights()
    gram_articles = dict(conn.execute(
        "SELECT gram, SUM(articles) FROM grams WHERE day>=? AND day<=? GROUP BY gram",
        (cw_start, cw_end)))
    corpus_articles = conn.execute(
        "SELECT SUM(articles) FROM totals WHERE day>=? AND day<=?",
        (cw_start, cw_end)).fetchone()[0] or 1
    ...
    for gram in candidates:
        ...
        rec_wk, pri_wk, _ = fortnights()           # week buckets, not day ranges
        recent_p, prior_p = {}, {}
        qmark = ",".join("?" * len(rec_wk))
        for o, n in conn.execute(
                "SELECT o, SUM(n) FROM ("
                f"  SELECT other o, n FROM cooc WHERE gram=? AND day IN ({qmark}) "
                "  UNION ALL "
                f"  SELECT gram o, n FROM cooc WHERE other=? AND day IN ({qmark})"
                ") GROUP BY o", tuple([gram] + rec_wk + [gram] + rec_wk)):
            if o not in recurring:
                continue
            recent_p[o] = n
        pmark = ",".join("?" * len(pri_wk))
        for o, n in conn.execute(
                "SELECT o, SUM(n) FROM ("
                f"  SELECT other o, n FROM cooc WHERE gram=? AND day IN ({pmark}) "
                "  UNION ALL "
                f"  SELECT gram o, n FROM cooc WHERE other=? AND day IN ({pmark})"
                ") GROUP BY o", tuple([gram] + pri_wk + [gram] + pri_wk)):
            prior_p[o] = n
        ...
        for o, _n in partners:
            words = [w for w in gram.split() + o.split() if len(w) > 3]
            if not words:
                continue
            # a stem for each word, so a folded plural still matches the source
            clause = " AND ".join(
                ["LOWER(title || '. ' || COALESCE(summary,'')) LIKE ?"] * len(words))
            together[o] = [{"title": ti, "url": u, "publisher": p, "sector": s}
                ...
                tuple([recent_start] + [f"%{w[:-1] if len(w) > 5 else w}%"
                                        for w in words]))]
        ...
        gwords = [w for w in gram.split() if len(w) > 3] or gram.split()
        gclause = " AND ".join(
            ["LOWER(title || '. ' || COALESCE(summary,'')) LIKE ?"] * len(gwords))
        examples = [{"publisher": p, "title": t, "url": u, "sector": s, "country": cc}
                    for p, t, u, s, cc in conn.execute(
            "SELECT publisher, title, url, sector, COALESCE(country,'US') FROM articles "
            f"WHERE published>=? AND {gclause} ORDER BY published DESC LIMIT 4",
            tuple([recent_start] + [f"%{w[:-1] if len(w) > 5 else w}%" for w in gwords]))]
    ...
    for a, b, n in conn.execute(
            "SELECT gram, other, SUM(n) FROM cooc "
            f"WHERE day IN ({','.join('?' * len(rwk))}) GROUP BY gram, other", tuple(rwk)):
