...
REACH_PEAK  = 3.5        # sectors at which a crossing reads as most notable
REACH_SPAN  = 4.5        # how slowly the credit eases away either side
...
MIN_PUBLISHERS = 3       # a phrase in two sectors carried by two outlets is
                         # one outlet per sector, which is not a crossing
...
SHOW_TERMS = 17          # how many reach the phrase list; the rest still feed
...
# words that qualify a subject rather than naming one, so two phrases sharing
# only one of these are not the same subject
MODIFIERS = set("""
low high new old big small large major minor key top first next last
global total full short long early late clean smart green
""".split())

SECTOR_BLOCS = [frozenset({"Energy", "Utilities", "Materials", "Waste & water"})]
FAMILY_MIN = 2            # phrases sharing a word this often are one subject
MAX_PER_FAMILY = 1        # places any one subject may take in the shown list
...
SYNONYM_FAMILIES = [
    ...
    {"charging network", "charging infrastructure", "charging point",
     "charging station"},
    {"electric vehicle", "new energy vehicle", "battery electric vehicle"},
]
...
COOC_MIN_SECTORS = 1     # a companion is context under a phrase, not a crossing
                         # of its own: the phrase it sits under already crossed, and a
                         # companion specific to one desk says what that desk pairs it
                         # with, which is the useful part
COOC_MIN_ARTICLES = 4    # a companion must recur; once is a sentence, not a topic
COOC_MIN_NEW = 2         # a first appearance carries no comparison, so it needs
...
# Nouns that cannot head a subject. Anything ending in one of these is a
...
another other others several various certain numerous
...
armed force forces combustion voltage fired development ion
...
def ngrams(text, nmax=3):
    ...
    for n in (2, 3):
        for i in range(len(words)-n+1):
            ...
            # the stored form is singularised, so a plural must be tested as one
            if singularise(" ".join(g)) in STOCK_PHRASES: continue
...
HOUSE_OF = {}          # masthead -> the domain that publishes it

def set_houses(cfg):
    """Which mastheads belong to one newsroom.

    Ten Economic Times verticals are ten names and one operation; counting
    them as ten publishers lets a phrase only they ran look widely carried.
    """
    HOUSE_OF.clear()
    for sec in cfg.get("sectors", []):
        for s in sec.get("sources", []):
            name = (s.get("publisher") or "").strip()
            url = s.get("url", "")
            host = url.split("/")[2].lower() if "://" in url else ""
            host = host[4:] if host.startswith("www.") else host
            # energy.economictimes.indiatimes.com -> economictimes.indiatimes.com
            parts = host.split(".")
            house = ".".join(parts[-3:]) if len(parts) > 3 else host
            if name:
                HOUSE_OF[name] = house or name

def house_of(name):
    return HOUSE_OF.get(name, name)
...
STOCK_PHRASES = {
    ...
    "economic uncertainty", "market volatility", "macroeconomic headwind",
    "mental health", "public health", "work life balance", "quality of life",
    "gender equality", "social justice", "cost of living", "food security",
    "human right", "best practice", "thought leadership", "duty of care",
    "cash flow", "working capital", "operating profit", "market share",
    "entry level", "social media", "fuel price", "rapidly expanding",
    # units, not subjects: "rose 25 basis points". The head rule cannot take
    # "point" because a charging point is a real thing.
    # outlets the corpus cites but does not read: attribution, not subject
    "street journal", "wall street journal report",
    ...
    # how a reporter attributes a source, not anything they are reporting
    "people familiar", "familiar with", "declined to comment", "did not respond",
    # produced by power, share and asset purchase agreements alike, so the
    # bigram claims a crossing between an energy contract and an M&A document
    "local time", "gmt time", "eastern time", "central time",
    "investment plan",
    "annual production", "annual revenue", "annual output",

    "speaking on condition", "on condition of", "according to people",
...
def company_vocabulary(companies):
    ...
    ordinary = GENERIC | STOP | PLACES | set("""
        ...
        climate green renewable renewables clean carbon hydrogen battery grid
      infrastructure capital partners asset assets management global
...
# Names that are also places. The word that follows says which is meant:
# Amazon Web Services is a company, the Amazon River is not.
PLACE_SENSE = {
    "amazon": ("river", "basin", "rainforest", "rainforests", "jungle",
               "delta", "region", "forest", "reef", "waterway"),
    "jordan": ("river", "valley"),
    "phoenix": ("arizona",),
    "cameron": ("highlands", "parish"),
    "orange": ("county", "river"),
}
...
def companies_in(text):
    ...
    low = text.lower()
    out = []
    for t, n, s, pat in COMPANIES:
        m = pat.search(text)
        if not m:
            continue
        senses = PLACE_SENSE.get(n.lower())
        if senses:
            # every mention has to be the place before we discard the name
            spots = [x for x in pat.finditer(text)]
            if all(
                (lambda after: after and after[0] in senses)(
                    re.findall(r"[a-z]+", low[x.end():x.end() + 24]))
                for x in spots
            ):
                continue
        out.append((t, n, s))
...
elon jensen satya sundar sam jamie pat tim jeff arvind akio carlos herbert
oliver warren lisa mary bill larry sergey jack marc reed dara lina gina
antonio ursula christine jerome janet mario mark sundar shantanu safra
""".split())
...
def store(conn, articles, sector, source, backfilled=False):
    ...
    for a in articles:
        ...
        low_url = (a.get("url") or "").lower()
        if any(mark in low_url for mark in SPONSORED):
            continue                    # an advertisement, not the desk writing
...
def rebuild(conn, verbose=True):
    ...
    """
    # anything older than the gram window is re-extracted and then pruned,
    # so the rebuild is bounded by retention rather than by the whole archive
    keep_from = (datetime.now(timezone.utc).date()
    ...
    for title, summary, sector, published, url, pub, cc in conn.execute(
            ...
            "COALESCE(country,'US') FROM articles WHERE published>=? "
            "ORDER BY published", (keep_from,)):
...
def write_digest(terms, path, stored, days_held):
    ...
    """
    today = datetime.now(timezone.utc).date()
    terms = [t for t in terms if t.get("display", True)]   # what the site shows
    ...
    L += ["## What is crossing now", ""]
    top = sorted(terms, key=lambda t: -t.get("rank", 0))[:6]
    for t in top:
        secs = ", ".join(sorted(s["name"] for s in t["sectors"])[:3])
        more = t["sectorCount"] - min(3, len(t["sectors"]))
        L.append(f"**[{t['term']}]({SITE}/#{t['id']})** \u2014 {secs}"
                 + (f" and {more} more" if more > 0 else "")
                 + f", {t['articles14d']} articles across "
                   f"{t['publishers']} newsrooms")
    L.append("")

    if crossed:
        L += ["## Crossed into a new sector this week", ""]
        for t in crossed[:6]:
            when = ("today" if t["daysSinceNewestSector"] == 0
                    else f"{t['daysSinceNewestSector']}d ago")
            first = min(t["sectors"], key=lambda s: s["firstSeen"])
            L.append(f"**{t['term']}** \u2014 now in {t['sectorCount']} sectors. "
                     f"**{t['newestSector']}** picked it up {when}; it was first "
                     f"recorded in {first['name'].lower()} coverage on "
                     f"{fmt(first['firstSeen'])}.")
            for e in t["examples"][:2]:
                L.append(f"  - [{e['title']}]({e['url']}) \u00b7 {e['publisher']}"
                         f" \u00b7 {e['sector'].lower()}")
            L.append("")
    ...
    if moving:
        ...
        for t in moving[:8]:
            secs = ", ".join(s["name"].lower() for s in t["sectors"][:3])
            L.append(f"**[{t['term']}]({SITE}/#{t['id']})** \u2014 "
                     f"{t['articles14d']} articles, "
                     f"{t['_growth']:.1f}\u00d7 the fortnight before, in {secs}")
            L.append("")

    # the most specific thing the site holds, and it is there every week
    withco = [t for t in terms if t.get("cooc")]
    if withco:
        L += ["## Being discussed alongside", ""]
        for t in sorted(withco, key=lambda t: -t.get("rank", 0))[:3]:
            names = ", ".join(c[0] for c in t["cooc"][:3])
            L.append(f"**{t['term']}** \u2014 {names}")
        L.append("")

    L += ["---", "",
          "Crosstalk tracks which phrases appear in more than one sector's trade press "
          "and when each sector started using them. It makes no forecast and "
          "reports only what has already been written: a multiple describes the "
          "fortnight just past, not the one ahead.", "",
          "crosstalkwire.com"]
...
def reach_weight(n):
    """How much credit a phrase gets for the number of sectors carrying it.

    Not a straight count: one sector is that sector's vocabulary and most
    sectors is everyone's, so the credit rises to a few desks and eases off.
    The everywhere gate already removes the far end; this stops the rank
    climbing towards it.
    """
    return math.exp(-((n - REACH_PEAK) ** 2) / (2 * REACH_SPAN ** 2)) * REACH_PEAK

def home_sector(t):
    """The desk a phrase takes the biggest share of.

    Not the desk with the most articles — a sector with nine publications
    outpublishes one with four on any subject, so raw counts describe the
    feed list rather than the phrase.
    """
...
def build(conn):
    ...
    # how much the whole corpus grew or shrank between the two fortnights, so
    # a pairing is not called falling because intake halved
    _rt = conn.execute("SELECT SUM(articles) FROM totals WHERE day>=?",
                       (recent_start,)).fetchone()[0] or 0
    _pt = conn.execute("SELECT SUM(articles) FROM totals WHERE day>=? AND day<?",
                       (days[-2 * RECENT_DAYS], recent_start)).fetchone()[0] or 0
    corpus_move = (_rt / _pt) if _pt else 1.0
    corpus_move = min(max(corpus_move, 0.25), 4.0)
    ...
    if sector_size:
        tot = sum(sector_size.values()) or 1
        print("  articles per sector · " + ", ".join(
            f"{k} {v:,}" for k, v in
            sorted(sector_size.items(), key=lambda kv: -kv[1])[:8]))

    # a month of history sits behind the visible window; anything in it was
    # already in use, whatever its first appearance inside the window says
    before_window = {g for (g,) in conn.execute(
    ...
    # keyed on the two words a follower must begin with, so each trigram
    # looks at its own candidates rather than at all of them
    starts = defaultdict(list)
    for b, bn in tri_list:
        bw = b.split()
        starts[(bw[0], bw[1])].append((b, bn, bw))
    for a, an in tri_list:
        if a in absorbed:
            continue
        aw = a.split()
        for b, bn, bw in starts.get((aw[1], aw[2]), ()):
            if a == b or b in absorbed:
                continue
    ...
    for gram in candidates:
        # a frame can sit under a phrase as context, but it cannot be one:
        # nothing ending in "management" or "capacity" names a subject
        if singularise(gram.split()[-1]) in FRAME_HEADS:
            frame_skipped.append(gram)
            continue
        if gram in joined:
        ...
        for s in sectors:
            # how much of this sector's own coverage the phrase takes, so a
            # large newsroom does not claim every phrase it happens to mention
            s["share"] = round(s["articles"] / max(1, sector_size.get(s["name"], 1)), 5)
        ...
        for s in sectors:
            ...
            # counted from today, like daysSinceNewestSector, so the two
            # figures for one phrase cannot disagree
            s["daysSince"] = (WINDOW_DAYS - 1) - s["adopt"]
        ...
        at_corpus_start = earliest <= max(horizon, add_days(window_start, 2))
        ...
        recent_p = {o: n for o, n in recent_p.items()
                    if o not in swallowed
                    and not (set(o.split()) & PLACE_ANCHORS)}
        ...
        _coocstage["proposed"] += len(chosen)
        ...
        _coocstage["after lift"] += len(lifted)
        ...
        for o, _n in partners:
            ...
            _coocstage["verified"] += 1
            rows2 = []
            pg, po = gap_pattern(gram), gap_pattern(o)
            def _both(r):
                text = f"{r[0]}. {r[4] or ''}"
                return pg.search(text) and po.search(text)
            for gf in like_forms(gram):
                for of in like_forms(o):
                    rows2 = [r for r in pair_rows([gf, of], 30) if _both(r)]
                    if rows2:
                        break
                if rows2:
                    break
            if not rows2:
                ...
                rows2 = [r for r in loose if _both(r)]
            if rows2:
                _coocstage["found articles"] += 1
                if len(rows2) >= 2:
                    _coocstage["two or more"] += 1
            ...
            if lead > COOC_FEED_SHARE and len(set(pubs2)) < 4:
                ...
                continue
            shown_n[o] = len(rows2)      # what we can actually put in front of a reader
            together[o] = [{"title": mark_in(ti, o), "url": u, "publisher": p,
                            "sector": s,
                            "quote": snippet(ti, sm, o) or snippet(ti, sm, gram)}
                           for ti, u, p, s, sm in rows2[:6]]
        ...
        # a companion whose articles are all inside another's is the same
        # observation said twice; fold it in and name it
        twins, folded = {}, set()
        by_url = {o: {a["url"] for a in arts} for o, arts in together.items() if arts}
        for o in sorted(by_url, key=lambda k: (-len(by_url[k]), k)):
            if o in folded:
                continue
            for p in sorted(by_url, key=lambda k: (-len(by_url[k]), k)):
                if p == o or p in folded or not by_url[p]:
                    continue
                if len(by_url[p]) <= len(by_url[o]) and by_url[p] <= by_url[o]:
                    twins.setdefault(o, []).append(p)
                    folded.add(p)
        ...
        def _window_rows():
            """The window read once, so a lookup is a scan of a list rather
            than another pass over the table."""
            if not _rows_cache:
                _rows_cache.extend(conn.execute(
                    "SELECT publisher, title, url, sector, COALESCE(country,'US'), "
                    "summary, published FROM articles WHERE published>=? "
                    "ORDER BY published DESC", (recent_start,)))
            return _rows_cache

        def _hay(r):
            return f"{r[1]}. {r[5] or ''}".lower()

        def _like_ok(hay, pattern):
            """The same test SQLite's LIKE does, on a string we already hold."""
        ...
        def articles_for(patterns, limit=12):
            out = []
            for r in _window_rows():
                h = _hay(r)
                if all(_like_ok(h, p) for p in patterns):
                    out.append(r)
                    if len(out) >= limit:
                        break
            return out
        ...
        # count newsrooms, not mastheads: sister verticals are one voice
        houses = defaultdict(int)
        for pname, cnt in pub_rows:
            houses[house_of(pname)] += cnt
        house_rows = sorted(houses.items(), key=lambda kv: -kv[1])
        pubs = len(house_rows)
        pub_total = sum(n for _, n in house_rows) or 1
        top_pub_share = (house_rows[0][1] / pub_total) if house_rows else None
        ...
        out.append({
            ...
            # the window edge is not a beginning: a phrase already in use
            # before the window was not picked up by anyone
            "pace": ("simultaneous" if adoption_span is not None
                     and adoption_span <= 7
                     and gram not in before_window
            ...
            "flow": flow,
            "cooc": [[o, round(n/mx, 2), pdir.get(o, "steady"), shown_n.get(o, n), prior_p.get(o, 0),
                      round(lift_of.get(o, 0), 1), together.get(o, []),
                      twins.get(o, [])]
                     for o, n in partners
                     if o not in drop_pairs and o not in folded],
            "examples": examples,
    ...
    phrase_move = normalise_directions(out)
    if abs(phrase_move - 1) > 0.08:
        print(f"  corpus itself moved {phrase_move:.2f}\u00d7 this fortnight; "
    ...
    for t in out:
        ...
        if t["docFreq"] >= COMMON_CEILING:
            ...
            everywhere += 1
        elif 0 < t.get("publishers", 0) < MIN_PUBLISHERS:
            t["display"] = False        # too few newsrooms to call it a crossing
            thin += 1
    ...
    def family_of(t):
        hits = [w for w in t["term"].split()
                if w not in MODIFIERS and word_n[w] >= FAMILY_MIN]
        return max(hits, key=lambda w: (word_n[w], w)) if hits else None

    fam_count = Counter()
    fam_shown_by = {}          # which phrase ended up standing for each subject
    lead_count = Counter()
    spread, held, pool = [], [], []
    for t in out:
        if not t["display"]:
    ...
    # tell the phrase that took the place what it is standing for
    for t in held:
        fam = family_of(t)
        rep = fam_shown_by.get(fam) if fam else None
    ...
    if frame_skipped:
        print(f"  {len(frame_skipped)} kept out of the list as frames "
              f"(still available as companions) · "
              + ", ".join(sorted(frame_skipped)[:8]))

    # now the list is settled, so "apart from the rest" means apart from what
    # a reader will see rather than from a pool most of which is hidden
    ...
    shown_now = [t for t in out if t.get("display", True) and t["sectors"]]
    if shown_now:
        firsts = sorted((min(s["firstSeen"] for s in t["sectors"]), t["term"])
                        for t in shown_now)[:4]
        print(f"  visible history · window opens {window_start}, "
              f"grams retained from {horizon}")
        print("  earliest first appearance among shown · " + ", ".join(
            f"{term} {d}" for d, term in firsts))
    if thin:
        print(f"  {thin} set aside as too few newsrooms "
              f"(fewer than {MIN_PUBLISHERS} carried them)")
    if places:
    ...
    lead_mix = Counter(home_sector(t)
    ...
    if mix:
        print("  companion directions · " + ", ".join(
            f"{d} {n}" for d, n in mix.most_common()))
    routes = Counter()
    for t in out:
        if not t.get("display", True):
            continue
        names = "/".join(sorted({s["name"] for s in t["sectors"]}))
        routes[(names, t["direction"])] += 1
    dupes = [(k, n) for k, n in routes.items() if n > 1]
    print(f"  distinct crossings · {len(routes)} routes for "
          f"{sum(routes.values())} phrases"
          + (" · shared: " + ", ".join(f"{a} {b} x{n}" for (a, b), n in dupes[:4])
             if dupes else " · no two phrases share one"))
    if lead_mix:
...
<footer>
<p><a href="{SITE}/#{t['id']}">Open this phrase in the dashboard</a> to see
...
def phase(name, t0):
    """Record how long a step took, so a slow run says which step was slow."""
    _PHASE[name] = _PHASE.get(name, 0.0) + (time.time() - t0)
    return time.time()
...
def print_phases():
    ...
    parts = ", ".join(f"{k} {v:.0f}s" for k, v in
                      sorted(_PHASE.items(), key=lambda kv: -kv[1]) if v >= 1)
    print(f"  time · {tot:.0f}s total · {parts}")
...
def main():
    ...
    print(f"crosstalk {VERSION} · rules v{EXTRACT_VERSION} · "
          f"code {hashlib.sha1(raw_code).hexdigest()[:8]}"
    ...
    # Stop before writing anything. A corrupt file that gets stored back
    # overwrites the last good copy, which is how one bad run becomes every
    # run after it.
    try:
        ...
        ok = f"unreadable: {e}"
    if ok != "ok":
        print(f"  database is damaged ({ok}) — stopping before anything is "
              f"written. Recover from the last good copy, or rebuild from "
              f"the articles table; every other table derives from it.")
        return 2
    ...
    if need_rebuild and have_v != EXTRACT_VERSION:
    ...
    # One rebuild covers both: the version change and any syndicated arrivals
    # this run removed. Doing it after the fetch means it is done once.
    t0 = time.time()
    if drop_syndicated(conn) or need_rebuild:
        rebuild(conn, verbose=need_rebuild)
        set_extract_version(conn)
        phase("rebuild", t0)

    days_held = conn.execute(
        "SELECT COUNT(DISTINCT substr(published,1,10)) FROM articles").fetchone()[0]
    ...
    try:
        ...
        for ticker, n in rows:
            ...
            attached = []
            for t in terms:
                # the same test the rest of the file uses: the corpus writes
                # "ride-hailing" where the stored phrase is "ride hailing"
                tp = gap_pattern(t["term"])
                hits = [x for x in titles
                        if tp.search(x[0] + " " + (x[1] or ""))]
                if hits:
