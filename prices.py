#!/usr/bin/env python3
"""
prices.py — free daily sector prices for Crosstalk. No key, no account.

Fetches daily closes for a sector ETF per Crosstalk sector, stored next to
...
UA   = "crosstalk-monitor/3.0 (sector price context; crosstalkwire.com)"

# Free keys, email signup only. Either one is enough; both are optional.
TWELVE_KEY = os.environ.get("TWELVEDATA_KEY", "").strip()
ALPHA_KEY  = os.environ.get("ALPHAVANTAGE_KEY", "").strip()

# One liquid sector ETF per Crosstalk sector. Policy, Waste & water and Labour have
...
def from_twelvedata(sym, timeout=40):
    """Twelve Data. Free key, 800 requests a day — ample for 11 daily symbols."""
    if not TWELVE_KEY:
        raise RuntimeError("TWELVEDATA_KEY not set")
    url = (f"https://api.twelvedata.com/time_series?symbol={sym}"
           f"&interval=1day&outputsize=5000&apikey={TWELVE_KEY}")
    doc = json.loads(_get(url, timeout))
    if doc.get("status") == "error" or "values" not in doc:
        raise RuntimeError(str(doc.get("message", doc))[:110])
    out = []
    for v in doc["values"]:
        try:
            out.append((v["datetime"][:10], float(v["close"])))
...
def from_alphavantage(sym, timeout=40):
    """Alpha Vantage. Free key, 25 requests a day — enough for 11 symbols."""
    if not ALPHA_KEY:
        raise RuntimeError("ALPHAVANTAGE_KEY not set")
    url = ("https://www.alphavantage.co/query?function=TIME_SERIES_DAILY"
           f"&symbol={sym}&outputsize=compact&apikey={ALPHA_KEY}")
    doc = json.loads(_get(url, timeout))
    series = doc.get("Time Series (Daily)")
    if not series:
        note = doc.get("Note") or doc.get("Information") or doc.get("Error Message") or doc
        raise RuntimeError(str(note)[:110])
    out = []
    for day, row in series.items():
        try:
            out.append((day[:10], float(row["4. close"])))
        except (KeyError, ValueError, TypeError):
            continue
    if not out:
        raise RuntimeError("no usable rows")
...
# Keyed providers first: they are the ones that actually answer from a
# datacentre. Yahoo and Stooq stay as free fallbacks in case a key is absent.
PLAIN = lambda s: s.replace(".us", "").upper()
PROVIDERS = [("twelvedata",   from_twelvedata,   PLAIN),
             ("alphavantage", from_alphavantage, PLAIN),
             ("yahoo",        from_yahoo,        PLAIN),
...
def probe():
    ...
    try:
        ...
        print("  nothing else in Crosstalk is affected. Send me the message.\n")
        return 1
    ...
    print("\n  Prices are stored but NOT shown in the dashboard. They are here so the")
    print("  prediction test is possible later without a setup delay — not as a")
    print("  feature. A price chart under a spread map implies causation that ten")
    print("  rounds of testing did not find.\n")
    return 0


def collect(conn, since_days=4000, verbose=True):
